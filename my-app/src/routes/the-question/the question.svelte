...
>
					...
					>
						{#key idx < corruptedCount ? `c-${p.id}-${corruptedCount}` : `n-${p.id}`}
							<img
								...
								src={pickSrc(p, idx)}
								alt={p.title}
								class={'h-44 w-44 rounded-xl object-cover select-none sm:h-52 sm:w-52 ' +
	...
	{#if mode === 'ask' || mode === 'corrupt' || mode === 'yes'}
		<div class="relative z-20 flex h-full items-center justify-center px-6">
			<div
				class="w-full max-w-2xl rounded-3xl bg-gray-900/75 p-10 text-center shadow-2xl backdrop-blur"
			>
				<h1 class="mb-4 text-4xl font-extrabold text-white sm:text-5xl">{titleText}</h1>

				<p
					class="mx-auto mb-10 max-w-xl text-2xl leading-snug font-semibold text-white sm:text-3xl"
				>
					{questionText}
				</p>
				...
				{#if mode !== 'yes'}
					<div class="flex flex-col gap-4 sm:flex-row sm:justify-center">
						...
						<button
							class="mx-auto w-full max-w-xs rounded-2xl bg-pink-500 px-8 py-4 text-2xl font-extrabold text-white shadow-lg transition hover:scale-[1.02] hover:bg-pink-600 disabled:opacity-60"
						...
						<!-- NON qui se “dégrade” -->
						<button
							class="mx-auto w-full max-w-xs rounded-2xl bg-gray-200/10 px-8 py-4 text-2xl font-extrabold text-white shadow-lg transition hover:bg-gray-200/20 disabled:opacity-60"
